...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m " +
           ...
           "LEFT JOIN FETCH m.attachments " +
           "WHERE ( " +
           "  (m.sender = :user AND m.receiver = :partner AND m.deletedBySender = false AND (m.isSystem = false OR m.visibleToUserId IS NULL OR m.visibleToUserId = :#{#user.id})) " +
           "  OR " +
           "  (m.sender = :partner AND m.receiver = :user AND m.deletedByReceiver = false AND (m.isSystem = false OR m.visibleToUserId IS NULL OR m.visibleToUserId = :#{#user.id})) " +
           ") " +
           "AND m.isDeleted = false")
